window.addEventListener('scroll', function () {
  ...
  const FagQuestionTop = document.querySelectorAll('.fag-question-top')
  
  for (let i = 0; i<FagQuestionTop.length; i++){
    FagQuestionTop[i].addEventListener('click', function(){
        const FagQuestionBottom = this.nextElementSibling

        if(FagQuestionBottom.style.maxHeight){
            FagQuestionBottom.style.maxHeight = null
            this.classList.remove('active')

        }
        else{
            FagQuestionBottom.style.maxHeight = FagQuestionBottom.scrollHeight+ "px"
